// resumeTemplate.js - Resume data and template management (FIXED for storage quota)
class ResumeTemplate {
  ...
  // Get current template (from LOCAL storage or default) - FIXED for quota
  async getCurrentTemplate() {
    try {
      // Use LOCAL storage for templates (higher quota - 10MB vs 8KB)
  ...
  // Save template to LOCAL storage (higher quota) - FIXED for quota
  async saveTemplate(template) {
    ...
    try {
      // Use LOCAL storage for large templates (10MB limit vs 8KB)
      await chrome.storage.local.set({ masterResume: template });
      console.log(
        "✅ Template saved to local storage, size:",
        template.length,
        "bytes"
      );
      return true;
  ...
  async resetToDefault() {
    try {
      await chrome.storage.local.set({ masterResume: this.defaultTemplate });
      return this.defaultTemplate;
    } catch (error) {
